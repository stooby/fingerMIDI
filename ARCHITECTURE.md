...
Control hover tooltips are data-driven from a single registry so strings can be edited in one place, without touching call sites. Lives in `ControlTooltips.swift`:

- **`ControlTooltips.all: [String: String]`** — the centralized `id → tooltip` map. Holds only custom strings and non-parameter buttons; multi-line tooltips embed `\n` (a real newline in a Swift literal, which macOS tooltips render as line breaks).
- **`ControlTooltips.text(for:)`** — resolves an id: explicit `all` entry → else the parameter's `ParameterStore.specs[id].label` (so plain knobs get a name-mirroring tooltip for free) → else `nil`.
- **`View.tooltip(_ id:)`** — applies `.help(Text(verbatim:))` when a non-empty string resolves, else passes the view through unchanged. `verbatim` keeps the string from being treated as a `LocalizedStringKey`.

**ID scheme:** parameter-backed controls reuse their RNBO `rnboId` (e.g. `Input_dB`, `Onset/enable`); controls with two distinct tooltips or no parameter use explicit ids (`SpecCentCutoff.range` / `SpecCentCutoff.slider`, `transport.play`, `export.midi`, `greyhole.randomize`). Call sites in `ContentView` and `HorizontalSliderView` attach `.tooltip("<id>")`; the spectral min/max icons, number boxes, and cutoff sliders all pull from the same registry.
